...
const graphAPIEndpoints = {
  ...
  limitOrders:
    'https://api.thegraph.com/subgraphs/name/synthetixio-team/synthetix-limit-orders',
  exchanger:
...
const graphWSEndpoints = {
  exchanges:
    'wss://api.thegraph.com/subgraphs/name/synthetixio-team/synthetix-exchanges',
  rates:
    'wss://api.thegraph.com/subgraphs/name/synthetixio-team/synthetix-rates',
};
  ...
  exchanges: {
    ...
    total({ network = 'mainnet' } = {}) {
      return pageResults({
        ...
        .then(
          ([
            { exchangers, exchangeUSDTally, totalFeesGeneratedInUSD, trades },
          ]) => ({
    ...
    since({
      ...
      return pageResults({
        ...
        query: {
          ...
          selection: {
            ...
            where: {
              network: `\\"${network}\\"`,
              timestamp_gte:
                roundTimestampTenSeconds(minTimestamp) || undefined,
              timestamp_lte:
                roundTimestampTenSeconds(maxTimestamp) || undefined,
        ...
        .then((results) =>
          results.map(module.exports.exchanges._mapSynthExchange)
        )
        .catch((err) => console.error(err));
    },
        ...
        subscribe({ next, error, complete }) {
          return observable.subscribe({
            next({ data: { synthExchanges } }) {
              synthExchanges
                .map(module.exports.exchanges._mapSynthExchange)
                .forEach(next);
            },
            error,
            complete,
      ...
      return pageResults({
        ...
        query: {
          ...
          selection: {
            ...
            where: {
              id:
                address && synth ? `\\"${address + '-' + synth}\\"` : undefined,
        ...
        .then((latestRates) => {
          const changeValues = latestRates.reduce((acc, curr) => {
            if (
              ...
              (synths.length === 0 ||
                (synths.length > 0 && synths.includes(curr.id)))
            ) {
              acc[curr.id] = {
        ...
        query: {
          ...
          selection: {
            ...
            where: {
              ...
              collateral_lte: maxCollateral
                ? `\\"${maxCollateral + '0'.repeat(18)}\\"`
                : undefined,
              collateral_gte: minCollateral
                ? `\\"${minCollateral + '0'.repeat(18)}\\"`
                : undefined,
          ...
          results.map(
            ({
              ...
              initialDebtOwnership: initialDebtOwnership
                ? initialDebtOwnership / 1e27
                : null,
              debtEntryAtIndex: debtEntryAtIndex
                ? debtEntryAtIndex / 1e27
                : null,
      ...
      return pageResults({
        ...
        query: {
          ...
          selection: {
            ...
            orderDirection: 'desc',
            where: {
              balanceOf_gte: minBalance
                ? `\\"${minBalance + '0'.repeat(18)}\\"`
                : undefined,
              balanceOf_lte: maxBalance
                ? `\\"${maxBalance + '0'.repeat(18)}\\"`
                : undefined,
            },
          },
  ...
  binaryOptions: {
    ...
    historicalOptionPrice({
      ...
      return pageResults({
        ...
        results.map(
          ({ id, timestamp, longPrice, shortPrice, poolSize, market }) => ({
            id,
            timestamp: Number(timestamp * 1000),
            longPrice: longPrice / 1e18,
            shortPrice: shortPrice / 1e18,
  ...
  liquidations: {
    ...
    getActiveLiquidations({
      ...
      return this.accountsFlaggedForLiquidation({
        ...
        this.accountsRemovedFromLiquidation({
          account,
          max,
          maxTime,
          minTime,
          ...
          return flaggedResults.reduce((acc, curr) => {
            if (
              removedResults.findIndex((o) => o.account === curr.account) === -1
            ) {
              acc.push(curr);
            }
            return acc;
