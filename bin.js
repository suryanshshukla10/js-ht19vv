...
program
  ...
  .action(async ({ minTimestamp, minBlock, max, fromAddress, json, csv }) => {
    const results = await exchanges.since({
      minTimestamp,
      minBlock,
      max,
      fromAddress,
...
const doReclaimRebates = ({ prg, isReclaim }) => {
  prg
    ...
    .action(async ({ minTimestamp, minBlock, max, account, json }) => {
      const results = await exchanges[isReclaim ? 'reclaims' : 'rebates']({
        minTimestamp,
        minBlock,
        max,
        account,
      ...
      const totalInUSD = results.reduce(
        (memo, { amountInUSD }) => memo + amountInUSD,
        0
      );
...
program
  ...
  .option(
    '-t, --type <value>',
    'The type of unit - months, weeks or days',
    'days'
  )
  .option(
    '-n, --unit <value>',
    'The number of units (months, weeks, days) back to include prior to the current',
    0
  )
  .option('-j, --json', 'Whether or not to display the results as JSON')
  .option('-c, --csv', 'Whether or not to display the results as a CSV')
  .action(async ({ type, unit, json, csv }) => {
    ...
    const groups = [];
    const _cache = {};
    const lastMomentInWindow = moment(results[0].timestamp).endOf(
      typeWithoutPlural
    );
...
program
  ...
  .action(async ({ max, addressesOnly, address, json, synth }) => {
    synths
      ...
      .then((results) =>
        addressesOnly ? results.map(({ address }) => address) : results
      )
      .then(logResults({ json }))
      .then(showResultCount({ max }));
  });
...
program
  ...
  .option('-j, --json', 'Whether or not to display the results as JSON')
  .option(
    '-t, --minTimestamp <value>',
    'The oldest timestamp to include, if any'
  )
  .option(
    '-T, --maxTimestamp <value>',
    'The youngest timestamp to include, if any'
  )
  .action(
    async ({
      max,
...
program
  ...
  .option(
    '-f, --fromBlock <value>',
    'will get rates 24HR prior starting from this block'
  )
  .option(
    '-m, --max <value>',
    'max needs to be higher than total synths in the system at the moment',
    100
  )
...
program
  ...
  .option('-m, --max <value>', 'Maximum number of results', 100)
  .option(
    '-n, --min-collateral <value>',
    'Minimum amount of collateral (input will have 18 decimals added)'
  )
  .option('-o, --addresses-only', 'Show addresses only')
  .option(
    '-x, --max-collateral <value>',
    'Maximum amount of collateral (input will have 18 decimals added)'
  )
  .action(
    async ({
      max,
...
program
  ...
  .option(
    '-c, --collateral-minted <value>',
    'The collateral minted for the loan'
  )
